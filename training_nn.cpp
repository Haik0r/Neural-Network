...
// Images in MNIST are 28x28 gray scale images
int d[width + 1][height + 1];

// Generate report of our input file
ifstream image;
ifstream label;
ofstream report;
...
void init_array() {
    ...
    for (int i = 1; i <= n1; ++i) {
        for (int j = 1; j <= n2; ++j) {
            ...
            // Another strategy to randomize the weights
...
int main(int argc, char *argv[]) {
	...
	// Save our  final trained network
    write_matrix(model_fn);
